...
bool Win32Window::Create(const std::wstring& title, const Point& origin, const Size& size)
{
   ...
   const wchar_t* wndClass = WindowClassRegistrar::GetInstance()->GetWindowClass();

   const POINT targetPoint = { static_cast<LONG>(origin.x), static_cast<LONG>(origin.y) };
   HMONITOR monitor = MonitorFromPoint(targetPoint, MONITOR_DEFAULTTONEAREST);
   UINT dpi = FlutterDesktopGetDpiForMonitor(monitor);
   double scaleFactor = dpi / 96.0;

   HWND window = CreateWindow(
      wndClass, title.c_str(), WS_OVERLAPPEDWINDOW,
...
// Create a method to load a PNG image as HBITMAP from resources
HBITMAP LoadImageFromResource(UINT resourceID)
{
   ...
   if (OleLoadPicture(pStream, imageSize, FALSE, IID_IPicture, reinterpret_cast<LPVOID*>(&hBitmap)) != S_OK)
   {
      return nullptr;
   }
